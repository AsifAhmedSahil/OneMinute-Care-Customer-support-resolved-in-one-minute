...
export async function POST(req:NextRequest) {
    try {
        const LoggedInUser = await isAuthorized();
            ...
            const pendingMember = await db.select().from(teamMembers).where(eq(teamMembers.user_email,LoggedInUser.email))
        ...
        console.log(error);
            return NextResponse.json("Failed to add team members", { status: 500 });
